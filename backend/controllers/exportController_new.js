...
class ExportController {
  ...
  async exportChildrenByTeam(req, res) {
    try {
      ...
      if (children.length === 0) {
        return res.status(404).json(formatError(`No hay niños registrados en el equipo "${team.nombre}"`, 404));
      }
      ...
      res.status(500).json(formatError('Error interno del servidor al exportar niños por equipo', 500));
    }
  }
  ...
  async exportCompleteReport(req, res) {
    try {
      ...
      res.status(500).json(formatError('Error interno del servidor al exportar reporte completo', 500));
    }
  }
  ...
  async exportChildrenCSV(req, res) {
    try {
      ...
      res.status(500).json(formatError('Error interno del servidor al exportar niños a CSV', 500));
    }
  }
